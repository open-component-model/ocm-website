...
| `content_versioned/version-vx.y.z` | Content snapshot for release version `vx.y.z`            |
...
```sh
# Create version v0.0.2 (and keep the current default)
npm run cutoff -- v0.0.2

# Create version v0.0.3 and set it as the default
npm run cutoff -- --makeDefault v0.0.3
```

What the script does (brief):
- Copies `content/` to `content_versioned/version-vx.y.z`.
- Appends a `[versions."vx.y.z"]` stanza (without weight) to `config/_default/hugo.toml`.
- Appends a grouped, per-version section to `config/_default/module.toml`.
- If `--makeDefault` is used, sets `defaultContentVersion = "vx.y.z"` in `hugo.toml` and mirrors it in `params.toml` for template usage.
