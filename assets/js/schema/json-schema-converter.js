...
/** Normalize type (may be array like ["string","null"]). */
...
export function jsonSchemaToModel(data) {
  ...
  if (root?.properties) {
    return createModel({
      ...
      sections: [createSection({ title: "Schema", fields: fieldsFrom(root, root, new Set()) })],
    });
  }
  ...
  for (const kw of ["oneOf", "anyOf"]) {
    if (Array.isArray(root?.[kw])) {
      ...
      return createModel({ meta: extractMeta(data, root), sections });
    }
  }
