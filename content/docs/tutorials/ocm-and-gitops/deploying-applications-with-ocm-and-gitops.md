...
To install the `ocm-controller` use the provided Helm chart from the `ocm-controller` GitHub project's ./deploy` folder. The Helm chart has also been uploaded to the GH container registry as OCI artifact.
